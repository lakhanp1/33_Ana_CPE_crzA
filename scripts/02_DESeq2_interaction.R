...
diffDataPath <- here::here("analysis", "01_RNAseq_Af293", "DESeq2_diff")
file_sampleInfo <- here::here("data", "reference_data", "RNAseq_sample_info.txt")
...
# analysisName <- "interaction_CEA17_dcrzA.2CAS"
...
exptInfo <- dplyr::tibble(sampleId = samples) %>% 
...
#   ) %>%
#   dplyr::group_by(condition, genotype, treatment) %>%
#   dplyr::summarise(sampleId = paste(sampleId, collapse = ";")) %>%
#   dplyr::arrange(genotype, treatment)

readr::write_tsv(x = exptInfo, file = paste(outDir, "/RNAseq_sample_info.txt", sep = ""))
...
resList <- list(
  ...
  interaction_II_B = list(
    ...
    label = "interaction_CEA17_dcrzA.2CAS",
    name = "genotypeCEA17_dcrzA.treatment2CAS",
    contrast = NA
  ),
  ## B vs A condition effect in genotype I
  gtI_BA = list(
    use = "name", type = "pairwise",
    label = "CEA17.2CAS_vs_C", 
    name = "treatment_2CAS_vs_C",
    contrast = NA
  ),
  ## B vs A condition effect in genotype II
  gtII_BA = list(
    use = "contrast", type = "pairwise",
    label = "CEA17_dcrzA.2CAS_vs_C", 
    name = NA,
    contrast = list(c("treatment_2CAS_vs_C", "genotypeCEA17_dcrzA.treatment2CAS"))
  )
)
...
# plotCounts(dds, gene="Afu1g00120", intgroup="condition")
# 
# gids <- c("Afu1g00120", "Afu1g00220", "Afu1g06670", "Afu1g01540", "Afu1g01740", "Afu2g05350", "Afu6g09370", "Afu1g03360", "Afu3g12920", "Afu5g10220", "Afu4g14175", "Afu6g04690")
# 
# dplyr::tibble(geneId = forcats::as_factor(gids)) %>% 
...
#   ) %>% 
#   dplyr::left_join(y = exptInfo, by = "sampleId") %>% 
#   dplyr::mutate(geneId = forcats::fct_relevel(.f = geneId, !!!gids)) %>% 
#   ggplot(mapping = aes(x = condition, y = normCount, color = treatment, shape = genotype)) +
#   geom_point(position=position_jitter(w=0.1,h=0), size = 2) +
#   scale_color_manual(
#     values = c("C" = "black", "02CAS" = "red")
#   ) +
#   facet_wrap(facets = ~geneId, nrow = 3, scales = "free_y") +
#   theme_bw() +
#   theme(
#     panel.grid = element_blank(),
#     legend.position = "bottom",
...
# 
# ggplot2::ggsave(
#   filename = "compare_counts.pdf", width = 8, height = 7
# )
