...
# analysisName <- "Af293_dcrzA_2CAS_vs_Af293_dcrzA_C"

## the denominator or WT in log2(fold_change) should be second
compare <- c("CEA17_dzipD_2CAS", "CEA17_2CAS")

analysisName <- paste(compare[1], "vs", compare[2], sep = "_")
...
excelOut <- paste(outPrefix, ".DEGs.xlsx", sep = "")
...
writeWorksheet(object = exc, data = degData, sheet = wrkSheet)
setAutoFilter(object = exc, sheet = wrkSheet,
              reference = aref(topLeft = "A1", dimension = dim(degData)))
...
#   countZscoreMat,
#   name = "count_heatmap",
#   col = colorRamp2(breaks = c(min(countZscoreMat, na.rm = T), 0, max(countZscoreMat, na.rm = T)),
